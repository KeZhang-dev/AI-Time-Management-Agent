...
function Kpi({ label, value, unit, sub }: { label: string; value: ReactNode; unit?: string; sub?: string }) {
  return (
    <div className="px-5.5 py-4.5 first:pl-0 last:pr-0">
      <div className="text-[11px] font-semibold tracking-wider text-faint-foreground uppercase">{label}</div>
...
export function KpiRow({ stats, recordCount }: KpiRowProps) {
  ...
  return (
    <div className="mb-11 grid grid-cols-2 divide-x divide-border rounded-lg border border-border px-5.5 sm:grid-cols-4">
      <Kpi label="Total tracked" value={stats ? stats.totalHours.toFixed(1) : "—"} unit="h" />
      <Kpi label="Records" value={recordCount} sub="all time" />
      <Kpi label="Categories" value={stats ? stats.byCategory.length : "—"} sub="in use" />
      <Kpi
        label="Top category"
        value={<span className="text-xl">{topCategory?.category ?? "—"}</span>}
        sub={topCategory ? `${topCategory.totalHours.toFixed(1)}h · ${topCategoryShare}% of total` : "no data yet"}
      />
    </div>
  );
}
